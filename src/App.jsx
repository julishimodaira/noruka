...
const WARN_NOTE = {
  rain: "Warning: Rain may affect outdoor ramps - allow extra time.",
  ice:  "Warning: Ice risk on ramps. Extra caution advised.",
  storm:"Warning: Storm conditions - check station alerts.",
  null: "Good conditions for travel.",
};

// Fallback static weather if API is unavailable
const WEATHER_FALLBACK = {
  tokyo:   {icon:"PartlyCloudy",temp:"14C",label:"Partly Cloudy",warn:null,note:"Good conditions."},
  osaka:   {icon:"Cloudy",temp:"16C",label:"Cloudy",warn:null,note:"Good conditions."},
  kyoto:   {icon:"Clear",temp:"12C",label:"Clear",warn:null,note:"Good conditions."},
  sapporo: {icon:"Snow",temp:"-2C",label:"Snow",warn:"ice",note:"Warning Ice risk on ramps. Extra caution."},
  fukuoka: {icon:"Clear",temp:"17C",label:"Clear",warn:null,note:"Good conditions."},
};
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.82)",zIndex:500,display:"flex",alignItems:"flex-end",justifyContent:"center",backdropFilter:"blur(4px)"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#0f1424",borderRadius:"20px 20px 0 0",border:"1px solid rgba(255,255,255,0.09)",width:"100%",maxWidth:700,maxHeight:"82vh",overflowY:"auto",padding:"18px 15px 40px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div style={{fontFamily:"monospace",fontSize:14,fontWeight:700,color:"#fff"}}>?? Japanese Phrase Cards</div>
    ...
    <div>
      ...
      {station.alerts?.map((a,i)=>(
        <div key={i} style={{display:"flex",gap:7,padding:"8px 11px",borderRadius:9,marginBottom:8,fontSize:11,lineHeight:1.4,background:a.type==="warning"?"rgba(245,158,11,0.1)":"rgba(66,133,244,0.08)",border:`1px solid ${a.type==="warning"?"rgba(245,158,11,0.28)":"rgba(66,133,244,0.22)"}`,color:a.type==="warning"?"#fde68a":"#93c5fd"}}>
          <span>{a.type==="warning"?"Warning:":"\u2139\ufe0f"}</span><span>{a.msg}</span>
        </div>
      ))}
      ...
      {tab==="comfort"&&(
        <div>
          ...
          <div style={{background:"rgba(16,185,129,0.07)",border:"1px solid rgba(16,185,129,0.18)",borderRadius:9,padding:"10px 12px",marginBottom:13}}>
            <div style={{fontSize:11,color:"rgba(255,255,255,0.65)",lineHeight:1.5}}>Multi-function toilets (??????) with grab bars, wide doors, and ostomate facilities are available at all stations scoring 3+. Follow ? signs from concourse.</div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:7}}><span style={{fontSize:16}}>??</span><span style={{fontSize:10,fontWeight:700,color:"#f87171",textTransform:"uppercase",letterSpacing:1}}>AED & Emergency</span></div>
...
- Use emoji sparingly but meaningfully (? for elevator, ? for train, ??? for staff, ? for accessibility note, Warning: for warnings, ? for tips)
